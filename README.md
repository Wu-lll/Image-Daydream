...
- **调用方式切换**：支持浏览器直连第三方 OpenAI 兼容节点，或通过本地桥接服务转发到第三方 `URL + API Key`。
- **批量生成**：单次可设置生成多张图片。
...
- **导入与导出**：支持将完整数据打包为 ZIP 导出。导出的 ZIP 内包含原始图片文件（非 base64）和记录图片元数据的 `manifest.json`，方便备份与迁移。出于安全考虑，导出的 `manifest.json` 不再包含 `API Key`。
...
3. **本地桥接服务（可选，推荐用于共享场景）**
   如果你想继续使用第三方 `URL + API Key`，但希望前端请求先走本地代理层，可以在项目根目录启动：
   ```bash
   npm run bridge
   ```
   默认监听：
   ...
   前端在“设置 → 调用方式”中切换到“本地桥接 URL + Key”后，填写：
   - `本地桥接 URL`：例如 `http://127.0.0.1:8765`
   - `上游 API URL`：你的第三方 OpenAI 兼容节点
   - `API Key`：第三方节点对应的 Key

   `local-bridge` 会把请求转发到你填写的第三方上游地址。  
   如果你希望桥接服务本身提供默认上游配置，可以复制：
   ```bash
   cp local-bridge/config.example.json local-bridge/config.json
   ```
   然后填入 `defaultBaseUrl`、`defaultApiKey`。默认也兼容读取本机 `C:\Users\Wu\Tools\OpenAI-Image\config` 下已有的配置文件。
   如需准备多套上游节点，可在 `fallbacks` 中配置备用 `baseUrl + apiKey`。桥接服务会先尝试默认上游，默认上游失败或没有返回可用图片时，再尝试备用上游。
...
- **调用方式**
  - **直连第三方 URL + Key**：浏览器直接请求你填写的第三方 OpenAI 兼容节点。
  - **本地桥接 URL + Key**：浏览器先请求本地 `local-bridge`，再由 `local-bridge` 转发到你填写的第三方 OpenAI 兼容节点。
- **API 预设**：内置两套成对的第三方 `URL + API Key`。选择预设会同时填入 URL、Key 和推荐调用方式，避免配置不匹配。
- **PPToken 注意事项**：PPToken 节点可通过本地桥接正常生图，但浏览器直连的 CORS 预检会被拒绝，因此预设默认使用“本地桥接”。
