...
function createRequestHeaders(settings: AppSettings): Record<string, string> {
  ...
  if (settings.transportMode === 'server') {
    headers['X-Image-Credential-Mode'] = settings.credentialMode
    headers['X-Image-Line'] = settings.providerLine
    if (settings.credentialMode === 'custom') {
      headers['X-OpenAI-Base-Url'] = settings.baseUrl
      headers['X-OpenAI-Api-Key'] = settings.apiKey
    }
    return headers
  }

  if (settings.transportMode === 'bridge') {
    headers['X-OpenAI-Base-Url'] = settings.baseUrl
    headers['X-OpenAI-Api-Key'] = settings.apiKey
    return headers
  }
...
function getRequestBaseUrl(settings: AppSettings): string {
  if (settings.transportMode === 'server') return '/api/openai/v1'
  return settings.transportMode === 'bridge' ? settings.bridgeUrl : settings.baseUrl
}

function buildRequestUrl(settings: AppSettings, path: string, proxyConfig?: ReturnType<typeof readClientDevProxyConfig>): string {
  if (settings.transportMode === 'server') {
    return `/api/openai/v1/${path.replace(/^\/+/, '')}`
  }
  return buildApiUrl(getRequestBaseUrl(settings), path, proxyConfig)
}
