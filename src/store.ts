  ...
  persist(
    (set, get) => ({
      ...
      setSettings: (s) => set((st) => ({
        settings: {
          ...
          ...s,
          credentialMode:
            s.credentialMode === 'site' || s.credentialMode === 'custom'
              ? s.credentialMode
              : st.settings.credentialMode ?? DEFAULT_SETTINGS.credentialMode,
          providerLine:
            s.providerLine === 'line1' || s.providerLine === 'line2'
              ? s.providerLine
              : st.settings.providerLine ?? DEFAULT_SETTINGS.providerLine,
          transportMode:
            (s.credentialMode ?? st.settings.credentialMode ?? DEFAULT_SETTINGS.credentialMode) === 'site'
              ? 'server'
              : s.transportMode === 'server' || s.transportMode === 'direct' || s.transportMode === 'bridge'
                ? s.transportMode
                : st.settings.transportMode ?? DEFAULT_SETTINGS.transportMode,
          apiMode:
          ...
          rememberCustomKey: s.rememberCustomKey ?? st.settings.rememberCustomKey ?? DEFAULT_SETTINGS.rememberCustomKey,
        },
      })),
    ...
    }),
    {
      ...
      partialize: (state) => ({
        settings: {
          ...state.settings,
          apiKey: state.settings.rememberCustomKey ? state.settings.apiKey : '',
        },
        params: state.params,
      ...
      merge: (persistedState, currentState) => {
        ...
        return {
          ...
          settings: {
            ...DEFAULT_SETTINGS,
            ...persistedSettings,
            baseUrl: persistedSettings.baseUrl?.trim() || DEFAULT_SETTINGS.baseUrl,
            apiKey: persistedSettings.rememberCustomKey ? persistedSettings.apiKey?.trim() || '' : '',
            bridgeUrl: persistedSettings.bridgeUrl?.trim() || DEFAULT_SETTINGS.bridgeUrl,
            credentialMode:
              persistedSettings.credentialMode === 'site' || persistedSettings.credentialMode === 'custom'
                ? persistedSettings.credentialMode
                : DEFAULT_SETTINGS.credentialMode,
            providerLine:
              persistedSettings.providerLine === 'line1' || persistedSettings.providerLine === 'line2'
                ? persistedSettings.providerLine
                : DEFAULT_SETTINGS.providerLine,
            transportMode:
              (persistedSettings.credentialMode ?? DEFAULT_SETTINGS.credentialMode) === 'site'
                ? 'server'
                : persistedSettings.transportMode === 'server' ||
                  persistedSettings.transportMode === 'direct' ||
                  persistedSettings.transportMode === 'bridge'
                  ? persistedSettings.transportMode
            ...
            rememberCustomKey: persistedSettings.rememberCustomKey ?? DEFAULT_SETTINGS.rememberCustomKey,
          },
        }
      },
    },
...
export async function submitTask() {
  ...
  if (settings.credentialMode === 'custom' && !settings.apiKey.trim()) {
    showToast('请先在设置中填写自己的 API Key', 'error')
  ...
  try {
    ...
    if (data.settings) {
      ...
      useStore.getState().setSettings({
        ...data.settings,
        apiKey: data.settings.apiKey?.trim() ? data.settings.apiKey : currentSettings.apiKey,
      })
    }
